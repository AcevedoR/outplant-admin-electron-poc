    ...
    for (const event_name in chain.events) {
        ...
        if (event.choices && event.choices.length > 0) {
            for (let i = 0; i < event.choices.length; i++) {
                ...
                nodes.push(create_node({event_id:choice_node_id, type:'outplantChoiceNode', choice:choice}))
...
function create_node(opt:{event_id: string, type?: string, event?: Event, choice?: Choice}): Node {
    return {
        id: opt.event_id,
